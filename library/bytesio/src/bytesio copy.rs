...
#[async_trait]
pub trait TNetIO: Send + Sync + MyToAny {
    ...
    async fn shut_down(self) -> Result<(), std::io::Error>;
...
impl TNetIO for TcpIO {
    ...
    async fn shut_down(self) -> Result<(), std::io::Error> {
        ...
        // let tcp_stream = stream.into_inner().into_std()?;

        let raw_fd = tcp_stream.as_raw_fd();

        // Drop the Tokio stream first
        drop(std::mem::replace(
            &mut tcp_stream,
            TcpStream::from_std(unsafe { std::net::TcpStream::from_raw_fd(-1) })
                .expect("dummy stream"),
        ));

        // Create a raw socket to send RST
        let socket = match Socket::new(Domain::IPV4, Type::STREAM, Some(Protocol::TCP)) {
            Ok(s) => s,
            Err(e) => return Err(BytesIOError { /* error conversion */ }),
        };

        // Set SO_LINGER to 0 for immediate RST
        socket.set_linger(Some(std::time::Duration::from_secs(0)))?;

        // Rebind the raw socket to the same file descriptor
        unsafe {
            socket.set_raw_fd(raw_fd);
        }

        // Closing the socket now will send RST
        drop(socket);
...
pub trait MyToAny: std::any::Any + 'static {
    fn into_any(self: Box<Self>) -> Box<dyn std::any::Any>;
}

impl<T: std::any::Any> MyToAny for T {
    fn into_any(self: Box<Self>) -> Box<dyn std::any::Any> {
        self
    }
}
